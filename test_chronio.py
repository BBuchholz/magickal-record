...
class TestChronIO(unittest.TestCase):
  ...
  def test_should_get_sabbat_as_date_stamp(self):
    sbt = self.cm.get_sabbat("BEL25")
    self.assertEqual("250501", sbt.as_date_stamp())
    sbt = self.cm.get_sabbat("LTH25")
    self.assertEqual("250621", sbt.as_date_stamp())
    sbt = self.cm.get_sabbat("LMS25")
    self.assertEqual("250801", sbt.as_date_stamp())
    sbt = self.cm.get_sabbat("MBN25")
    self.assertEqual("250921", sbt.as_date_stamp())
    sbt = self.cm.get_sabbat("SMH25")
    self.assertEqual("251031", sbt.as_date_stamp())
    sbt = self.cm.get_sabbat("YUL25")
    self.assertEqual("251221", sbt.as_date_stamp())
    sbt = self.cm.get_sabbat("IMB26")
    self.assertEqual("Imbolc 2026", sbt.as_date_stamp())
    sbt = self.cm.get_sabbat("OST26")
    self.assertEqual("Ostara 2026", sbt.as_date_stamp())
    sbt = self.cm.get_sabbat("BEL26")
    self.assertEqual("Beltane 2026", sbt.as_date_stamp())
    sbt = self.cm.get_sabbat("LTH26")
    self.assertEqual("Litha 2026", sbt.as_date_stamp())
    sbt = self.cm.get_sabbat("LMS26")
    self.assertEqual("Lammas 2026", sbt.as_date_stamp())
    sbt = self.cm.get_sabbat("MBN26")
    self.assertEqual("Mabon 2026", sbt.as_date_stamp())
    sbt = self.cm.get_sabbat("SMH26")
    self.assertEqual("Samhain 2026", sbt.as_date_stamp())
    sbt = self.cm.get_sabbat("YUL26")
    self.assertEqual("Yule 2026", sbt.as_date_stamp())

  def test_should_get_sabbat_as_string_code(self):
    sbt = self.cm.get_sabbat("BEL25")
    self.assertEqual("BEL25", sbt.as_string_code())
    sbt = self.cm.get_sabbat("LTH25")
    self.assertEqual("LTH25", sbt.as_string_code())
    sbt = self.cm.get_sabbat("LMS25")
    self.assertEqual("LMS25", sbt.as_string_code())
    sbt = self.cm.get_sabbat("MBN25")
    self.assertEqual("MBN25", sbt.as_string_code())
    sbt = self.cm.get_sabbat("SMH25")
    self.assertEqual("SMH25", sbt.as_string_code())
    sbt = self.cm.get_sabbat("YUL25")
    self.assertEqual("YUL25", sbt.as_string_code())
    sbt = self.cm.get_sabbat("IMB26")
    self.assertEqual("IMB26", sbt.as_string_code())
    sbt = self.cm.get_sabbat("OST26")
    self.assertEqual("OST26", sbt.as_string_code())
    sbt = self.cm.get_sabbat("BEL26")
    self.assertEqual("BEL26", sbt.as_string_code())
    sbt = self.cm.get_sabbat("LTH26")
    self.assertEqual("LTH26", sbt.as_string_code())
    sbt = self.cm.get_sabbat("LMS26")
    self.assertEqual("LMS26", sbt.as_string_code())
    sbt = self.cm.get_sabbat("MBN26")
    self.assertEqual("MBN26", sbt.as_string_code())
    sbt = self.cm.get_sabbat("SMH26")
    self.assertEqual("SMH26", sbt.as_string_code())
    sbt = self.cm.get_sabbat("YUL26")
    self.assertEqual("YUL26", sbt.as_string_code())
